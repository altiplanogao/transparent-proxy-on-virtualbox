...
# 给 UDP 打标记 1，转发至 %PROXY_TRANSP_PORT% 端口
iptables -t mangle -A V2RAY -p udp -j TPROXY --on-port %PROXY_TRANSP_PORT% --tproxy-mark 1
# 给 TCP 打标记 1，转发至 %PROXY_TRANSP_PORT% 端口
iptables -t mangle -A V2RAY -p tcp -j TPROXY --on-port %PROXY_TRANSP_PORT% --tproxy-mark 1
